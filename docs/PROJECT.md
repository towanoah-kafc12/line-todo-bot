...
LINE Bot を家の共有 TODO の唯一の UI として扱い、Todoist の特定 project 配下の共有 section 群を正本にしたまま、非 IT 利用者でも迷わず TODO を見て更新できる状態を作る。
...
重要なのは、LINE と Todoist が別々の TODO を持つことではなく、同じ project 配下の共有 section 群を同じ対象として扱い続けることだ。
...
- デフォルト rich menu と状態別 per-user rich menu を使って、個人チャットとグループの両方で主要操作の入口を出せる
- Todoist の特定 project 配下の複数 section の active tasks を一覧取得できる
...
- [x] 最低限の検証方針が決まっている
- [x] 運用や保守に必要な情報が残っている
- [ ] LINE からの 5 コマンドが対象 project 配下の共有 section 群に対して正しく作用する
...
- 既存システム制約: Todoist を正本にし、対象 project 配下の設定済み section 以外は操作対象にしない
...
| 2026-04-15 | 0.5 | 会話中だけ per-user rich menu へ切り替える UX を追加 | Q-APP-004 |
