...
リッチメニューは、通常時の main menu を section ごとの 2 カラムに分けて、各 section の `一覧を見る` `追加する` `編集する` `完了する` を直接押せる方針にする。
`一覧を見る` はその section の一覧を即時表示し、`追加する` は section を固定したまま入力待ちへ進む。
`編集する` と `完了する` は、その section の番号付き一覧を出して次の入力へつなぐ。
通常時は default main menu を出し、会話 state がある間だけ per-user rich menu を user 単位で上書きする。
理由は、2 section 前提なら main menu で section を固定してしまった方が 1 手減り、同じ操作でも対象 section を見失いにくいからだよ。
main menu と section 選択メニューの postback に使う `section_id` は、rich menu 適用時に `.env` の `TODOIST_SECTION_IDS` / `TODOIST_SECTION_NAMES` から組み立てる。
...
- main menu の section 固定ボタン、または add 用 state menu から section を決める
- そのあと内容を受け取って、選ばれた `section_id` に task を追加する
- 追加後の task と section 名を元に成功メッセージを返す

### `編集`

- main menu の section 固定ボタン、または edit 用 state menu から対象 section を決める
- その section の番号付き一覧を出して、対象 task を選ばせる
- task 選択後は、新しい内容を送るか `削除` を送る
- `削除` が来たら edit flow 内で delete API を呼ぶ
...
| 2026-04-16 | 0.8 | main menu を section 固定の 2 カラムへ変更し、削除を編集 flow に統合 | Q-APP-004 |
