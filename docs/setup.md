...
```bash
curl -X GET https://api.todoist.com/rest/v1/projects \
...
- Todoist の current docs は `https://developer.todoist.com/api/v1/` に集約されつつあり、古い REST reference 例は `/rest/v1` を表示している
...
curl -s -H "Authorization: Bearer $TODOIST_API_TOKEN" \
  "https://api.todoist.com/rest/v1/sections?project_id=$TODOIST_PROJECT_ID"
```
